 ...
 * be handler via the dedicated signal manager thread. Those go directly to
 * the thread that caused the exception and if blocked, it *may* be never
 * received, based on the platform you will use. Not delivering such signals can
 * cause serious problems - you might be running with corrupted memory, for
 * example, without knowing it.
 *
