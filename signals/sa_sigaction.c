...
term_handler(int sig, siginfo_t *info, void *ignored)
{
	 ...
	 *
	 * Note that SIGTERM should never be received otherwise than a user
	 * process, that's what the signal is for, but let's do it anyway. If
	 * curious see what exactly si_code is for and what it can carry. Also,
	 * you may want to read more about sigqueue(3C) with its 2nd parameter.
	 */
	if (info->si_code <= 0) {
